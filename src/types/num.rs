...
impl<Scalar: PrimeField> Clone for Num<Scalar> {
	fn clone(&self) -> Self {
		Num { value: self.value, variable: self.variable }
	}
}
...
impl<Scalar: PrimeField> Num<Scalar> {
		...
		F: FnOnce() -> Result<Scalar, SynthesisError>,
	{
		...
		Ok(Num { value: new_value, variable: var })
	}
		...
		CS: ConstraintSystem<Scalar>,
	{
		...
		for b in b.iter().by_vals().rev() {
			...
			if b {
				...
				let a_bit = Bit::alloc(cs.namespace(|| format!("bit {}", i)), a_bit)?;
				...
				let a_bit = Bit::alloc_conditionally(
	...
	where
		...
		CS: ConstraintSystem<Scalar>,
	{
		let bits = field_into_bits_le(&mut cs, self.value)?;
...
pub struct UnallocatedNum<Scalar: PrimeField> {
	value: Option<Scalar>,
	lc: LinearCombination<Scalar>,
}

impl<Scalar: PrimeField> From<Num<Scalar>> for UnallocatedNum<Scalar> {
	fn from(num: Num<Scalar>) -> UnallocatedNum<Scalar> {
		UnallocatedNum { value: num.value, lc: LinearCombination::<Scalar>::zero() + num.variable }
	}
}

impl<Scalar: PrimeField> UnallocatedNum<Scalar> {
	pub fn zero() -> Self {
		UnallocatedNum { value: Some(Scalar::zero()), lc: LinearCombination::zero() }
	}
	...
	pub fn add_bool_with_coeff(self, one: Variable, bit: &Boolean, coeff: Scalar) -> Self {
		...
		UnallocatedNum { value: newval, lc: self.lc + &bit.lc(one, coeff) }
	}
}
...
mod test {
	...
	fn test_num_alloc() {
		...
		Num::alloc(&mut cs, || Ok(Scalar::one())).unwrap();
	...
	fn test_num_squaring() {
		...
		let n = Num::alloc(&mut cs, || Ok(Scalar::from(3))).unwrap();
	...
	fn test_num_multiplication() {
		...
		let n = Num::alloc(cs.namespace(|| "a"), || Ok(Scalar::from(12))).unwrap();
		let n2 = Num::alloc(cs.namespace(|| "b"), || Ok(Scalar::from(10))).unwrap();
	...
	fn test_num_nonzero() {
		...
		}
		{
			...
			let n = Num::alloc(&mut cs, || Ok(Scalar::zero())).unwrap();
	...
	fn test_into_bits_strict() {
		...
		let n = Num::alloc(&mut cs, || Ok(negone)).unwrap();
	...
	fn test_into_bits() {
		...
		for i in 0..200 {
			...
			let n = Num::alloc(&mut cs, || Ok(r)).unwrap();
