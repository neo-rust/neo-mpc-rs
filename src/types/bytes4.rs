...
impl Bytes4 {
		...
		CS: ConstraintSystem<Scalar>,
	{
		...
		let bits = values
			...
			.enumerate()
			.map(|(i, v)| {
				Ok(Boolean::from(Bit::alloc(cs.namespace(|| format!("allocated bit {}", i)), v)?))
			})
	...
	pub fn from_bits_be(bits: &[Boolean]) -> Self {
		...
		Bytes4 { value, bits: bits.iter().rev().cloned().collect() }
	}
	...
	pub fn rotr(&self, by: usize) -> Self {
		...
		Bytes4 { bits: new_bits, value: self.value.map(|v| v.rotate_right(by as u32)) }
	}
	...
	pub fn shr(&self, by: usize) -> Self {
		...
		Bytes4 { bits: new_bits, value: self.value.map(|v| v >> by as u32) }
	}
		...
		U: Fn(&mut CS, usize, &Boolean, &Boolean, &Boolean) -> Result<Boolean, SynthesisError>,
	{
		...
		Ok(Bytes4 { bits, value: new_value })
	}
		...
		M: ConstraintSystem<Scalar, Root = MultiEq<Scalar, CS>>,
	{
		...
		Ok(Bytes4 { bits: result_bits, value: modular_value })
	}
}
...
mod test {
	use super::Bytes4;
	use crate::{helpers::multieq::MultiEq, types::Boolean};
