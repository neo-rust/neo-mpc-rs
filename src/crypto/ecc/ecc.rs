...
pub struct EdwardsPoint {
    u: Num<bls12_381::Scalar>,
    v: Num<bls12_381::Scalar>,
}
...
impl EdwardsPoint {
    pub fn get_u(&self) -> &Num<bls12_381::Scalar> {
        &self.u
    }

    pub fn get_v(&self) -> &Num<bls12_381::Scalar> {
        &self.v
    }
        ...
        CS: ConstraintSystem<bls12_381::Scalar>,
    {
        ...
        // Allocate u
        let u = Num::alloc(cs.namespace(|| "u"), || Ok(p.get()?.get_u()))?;

        // Allocate v
        let v = Num::alloc(cs.namespace(|| "v"), || Ok(p.get()?.get_v()))?;
        ...
        CS: ConstraintSystem<bls12_381::Scalar>,
    {
        ...
        let u_prime = Num::alloc(cs.namespace(|| "u'"), || {
        ...
        let v_prime = Num::alloc(cs.namespace(|| "v'"), || {
            if *condition.get_value().get()? {
        ...
        CS: ConstraintSystem<bls12_381::Scalar>,
    {
        ...
        //           = (u + v) * (u + v)
        let t = Num::alloc(cs.namespace(|| "T"), || {
        ...
        // Compute C = d*A*A
        let c = Num::alloc(cs.namespace(|| "C"), || {
        ...
        // Compute u3 = (2.A) / (1 + C)
        let u3 = Num::alloc(cs.namespace(|| "u3"), || {
        ...
        //            = (T - 2.A) / (1 - C)
        let v3 = Num::alloc(cs.namespace(|| "v3"), || {
            let mut t0 = *a.get_value().get()?;
            t0 = t0.double().neg();
        ...
        CS: ConstraintSystem<bls12_381::Scalar>,
    {
        ...
        let uppercase_u = Num::alloc(cs.namespace(|| "U"), || {
            let mut t0 = *self.u.get_value().get()?;
            t0.add_assign(self.v.get_value().get()?);
...
impl MontgomeryPoint {
    ...
    {
        // Compute u = (scale*x) / y
        let u = Num::alloc(cs.namespace(|| "u"), || {
        ...
        // Compute v = (x - 1) / (x + 1)
        let v = Num::alloc(cs.namespace(|| "v"), || {
            let mut t0 = *self.x.get_value().get()?;
            let mut t1 = t0;
        ...
        CS: ConstraintSystem<bls12_381::Scalar>,
    {
        ...
        let lambda = Num::alloc(cs.namespace(|| "lambda"), || {
        ...
        let xprime = Num::alloc(cs.namespace(|| "xprime"), || {
        ...
        let yprime = Num::alloc(cs.namespace(|| "yprime"), || {
...
mod test {
    ...
    lazy_static! {
        static ref EXAMPLE_GENERATOR_REF: FixedGeneratorOwned =
            generate_circuit_generator(EXAMPLE_GENERATOR);
    }
    ...
    fn test_into_edwards() {
        ...
        for _ in 0..100 {
            ...
            let numx = Num::alloc(cs.namespace(|| "mont x"), || Ok(x)).unwrap();
            let numy = Num::alloc(cs.namespace(|| "mont y"), || Ok(y)).unwrap();
    ...
    fn test_interpret() {
        ...
        for _ in 0..100 {
            let p = ExtendedPoint::random(&mut rng).to_affine();
            let (u, v) = (p.get_u(), p.get_v());

            let mut cs = TestConstraintSystem::new();
            let numu = Num::alloc(cs.namespace(|| "u"), || Ok(u)).unwrap();
            let numv = Num::alloc(cs.namespace(|| "v"), || Ok(v)).unwrap();
    ...
    fn test_edwards_fixed_base_multiplication() {
        ...
        for _ in 0..100 {
            ...
            let p = EXAMPLE_GENERATOR;
            let s = Fr::random(&mut rng);
            let q = ExtendedPoint::from(p * s).to_affine();
    ...
    fn test_edwards_multiplication() {
        ...
        for _ in 0..100 {
            ...
            let num_u0 = Num::alloc(cs.namespace(|| "u0"), || Ok(u0)).unwrap();
            let num_v0 = Num::alloc(cs.namespace(|| "v0"), || Ok(v0)).unwrap();
    ...
    fn test_edwards_addition() {
        ...
        for _ in 0..100 {
            let p1 = ExtendedPoint::random(&mut rng);
            let p2 = ExtendedPoint::random(&mut rng);
            ...
            let num_u1 = Num::alloc(cs.namespace(|| "u1"), || Ok(u1)).unwrap();
            let num_v1 = Num::alloc(cs.namespace(|| "v1"), || Ok(v1)).unwrap();
    ...
    fn test_montgomery_addition() {
        ...
        for _ in 0..100 {
            ...
            let num_x0 = Num::alloc(cs.namespace(|| "x0"), || Ok(x0)).unwrap();
            let num_y0 = Num::alloc(cs.namespace(|| "y0"), || Ok(y0)).unwrap();

            let num_x1 = Num::alloc(cs.namespace(|| "x1"), || Ok(x1)).unwrap();
            let num_y1 = Num::alloc(cs.namespace(|| "y1"), || Ok(y1)).unwrap();
    ...
    fn test_assert_not_small_order() {
        let check_small_order_from_p = |p: ExtendedPoint, is_small_order| {
            let mut cs = TestConstraintSystem::new();
        ...
        let prime_subgroup_order = Fr::from_str_vartime(
            "6554484396890773809930967563523245729705921265872317281365359162392183254199",
        )
        .unwrap();
        let largest_small_subgroup_order = Fr::from(8);
        ...
        let g = AffinePoint::from_raw_unchecked(u, v).into();
        ...
        let prime_subgroup_order_minus_1 = prime_subgroup_order - Fr::one();
        ...
        let largest_small_subgroup_order_minus_1 = largest_small_subgroup_order - Fr::one();
