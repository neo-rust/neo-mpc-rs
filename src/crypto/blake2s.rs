...
fn blake2s_compression<Scalar: PrimeField, CS: ConstraintSystem<Scalar>>(
    ...
    */
    ...
    v.push(Bytes4::constant(0x6A09E667));
    v.push(Bytes4::constant(0xBB67AE85));
    v.push(Bytes4::constant(0x3C6EF372));
    v.push(Bytes4::constant(0xA54FF53A));
    v.push(Bytes4::constant(0x510E527F));
    v.push(Bytes4::constant(0x9B05688C));
    v.push(Bytes4::constant(0x1F83D9AB));
    v.push(Bytes4::constant(0x5BE0CD19));

    assert_eq!(v.len(), 16);

    v[12] = v[12].xor(cs.namespace(|| "first xor"), &Bytes4::constant(t as u32))?;
    v[13] = v[13].xor(
        cs.namespace(|| "second xor"),
        &Bytes4::constant((t >> 32) as u32),
    )?;

    if f {
        v[14] = v[14].xor(
            cs.namespace(|| "third xor"),
            &Bytes4::constant(u32::max_value()),
        )?;
    }
...
pub fn blake2s<Scalar: PrimeField, CS: ConstraintSystem<Scalar>>(
    ...
    let mut h = vec![
        Bytes4::constant(0x6A09E667 ^ 0x01010000 ^ 32),
        Bytes4::constant(0xBB67AE85),
        Bytes4::constant(0x3C6EF372),
        Bytes4::constant(0xA54FF53A),
        Bytes4::constant(0x510E527F),
        Bytes4::constant(0x9B05688C),
        // Personalization is stored here
        Bytes4::constant(0x1F83D9AB ^ LittleEndian::read_u32(&personalization[0..4])),
        Bytes4::constant(0x5BE0CD19 ^ LittleEndian::read_u32(&personalization[4..8])),
    ];

    let mut blocks: Vec<Vec<Bytes4>> = vec![];
    ...
    for block in input.chunks(512) {
        ...
        for word in block.chunks(32) {
            ...
            this_block.push(Bytes4::from_bits(&tmp));
        }
        while this_block.len() < 16 {
            this_block.push(Bytes4::constant(0));
        }
        blocks.push(this_block);
    }

    if blocks.is_empty() {
        blocks.push((0..16).map(|_| Bytes4::constant(0)).collect());
    }
...
mod test {
    ...
    fn test_blake2s_constraints() {
        ...
        let input_bits: Vec<_> = (0..512)
            .map(|i| {
                Bit::alloc(cs.namespace(|| format!("input bit {}", i)), Some(true))
                    .unwrap()
                    .into()
            })
            .collect();
