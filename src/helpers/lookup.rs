...
pub fn synth<'a, Scalar: PrimeField, I>(window_size: usize, constants: I, assignment: &mut [Scalar])
where
	...
	CS: ConstraintSystem<Scalar>,
{
	...
	let res_x = Num::alloc(cs.namespace(|| "x"), || Ok(coords[*i.get()?].0))?;

	// Allocate the y-coordinate resulting from the lookup
	let res_y = Num::alloc(cs.namespace(|| "y"), || Ok(coords[*i.get()?].1))?;

	// Compute the coefficients for the lookup constraints
	let mut x_coeffs = [Scalar::from(0); 8];
	let mut y_coeffs = [Scalar::from(0); 8];
...
pub fn lookup3_xy_with_conditional_negation<Scalar: PrimeField, CS>(
	...
	bits: &[Boolean],
	coords: &[(Scalar, Scalar)],
) -> Result<(UnallocatedNum<Scalar>, UnallocatedNum<Scalar>), SynthesisError>
where
	CS: ConstraintSystem<Scalar>,
{
	assert_eq!(bits.len(), 3);
	...
	let y = Num::alloc(cs.namespace(|| "y"), || {
...
mod test {
	...
	fn test_lookup3_xy() {
		...
		for _ in 0..100 {
			...
			let a = Boolean::from(Bit::alloc(cs.namespace(|| "a"), Some(a_val)).unwrap());

			let b_val = rng.next_u32() % 2 != 0;
			let b = Boolean::from(Bit::alloc(cs.namespace(|| "b"), Some(b_val)).unwrap());

			let c_val = rng.next_u32() % 2 != 0;
			let c = Boolean::from(Bit::alloc(cs.namespace(|| "c"), Some(c_val)).unwrap());

			let bits = vec![a, b, c];

			let points: Vec<(Scalar, Scalar)> =
				(0..8).map(|_| (Scalar::random(&mut rng), Scalar::random(&mut rng))).collect();
	...
	fn test_lookup3_xy_with_conditional_negation() {
		...
		for _ in 0..100 {
			...
			let points: Vec<(Scalar, Scalar)> =
				(0..4).map(|_| (Scalar::random(&mut rng), Scalar::random(&mut rng))).collect();
	...
	fn test_lookup_synth() {
		...
		let constants: Vec<_> = (0..(1 << window_size)).map(|_| Scalar::random(&mut rng)).collect();
